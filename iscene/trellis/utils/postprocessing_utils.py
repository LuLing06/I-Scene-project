...
def _fill_holes(
    ...
    # Rasterize
    visibility = torch.zeros(faces.shape[0], dtype=torch.int32, device=verts.device)
...
def bake_texture(
    ...
    if mode == 'fast':
        ...
        rastctx = utils3d.torch.RastContext(backend='cuda')
        for k, (observation, view, projection) in enumerate(tqdm(zip(observations, views, projections), total=len(observations), disable=not verbose, desc='Texture baking (fast)')):
            with torch.no_grad():
                rast = utils3d.torch.rasterize_triangle_faces(
                    rastctx, vertices[None], faces, observation.shape[1], observation.shape[0], uv=uvs[None], view=view, projection=projection
                )
                uv_map = rast['uv'][0].detach().flip(0)
                mask = rast['mask'][0].detach().bool() & masks[k]
...
def to_glb(
    ...
    try:
        ...
        # Use GS as-is for baking; do not modify GS points.
        app_rep_canon = app_rep
        # Extract GS center/scale for alignment-only transform of mesh
        gs_scene = app_rep.get_xyz
        gs_scene_np = gs_scene.detach().cpu().numpy()
        gs_scene_center = (gs_scene_np.min(axis=0) + gs_scene_np.max(axis=0)) * 0.5
        gs_scene_scale = 1.0 / float(np.maximum(gs_scene_np.max(axis=0) - gs_scene_np.min(axis=0), 1e-8).max())
        # We do not change GS; only align mesh to GS space.

        # Align mesh to GS scene space without changing GS:
        # 1) canonicalize mesh (own bbox), 2) optional padding, 3) map to GS scene using GS center/scale
        scale_padding = 1.0
        mesh_min = vertices.min(axis=0)
        mesh_max = vertices.max(axis=0)
        mesh_center = (mesh_min + mesh_max) * 0.5
        mesh_scale = 1.0 / float(np.maximum(mesh_max - mesh_min, 1e-8).max())
        canonical_vertices = (vertices - mesh_center) * mesh_scale
        canonical_vertices = canonical_vertices * scale_padding
        canonical_vertices = canonical_vertices / gs_scene_scale + gs_scene_center
